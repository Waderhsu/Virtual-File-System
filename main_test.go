...
func TestCaseInsensitive(t *testing.T) {
	...
	for _, test := range tests {
		handleCommand(test.input)
		// test the case insensitive username
		if _, exists := users["testuser"]; !exists {
			t.Errorf("User 'TESTUSER' and 'testuser' should be the same user.")
		}
	}
	// test the case insensitive foldername
	if _, exists := users["testuser"].Folders["testfolder"]; !exists {
		t.Errorf("Folder 'TESTFOLDER' and 'testfolder' should be the same folder.")
	}
	// test the case insensitive filename
	if _, exists := users["testuser"].Folders["testfolder"].Files["testfile"]; !exists {
		t.Errorf("File 'TESTFILE' and 'testfile' should be the same file.")
	}

	// test the delete file command
	handleCommand("delete-file TESTUSER TESTFOLDER TESTFILE")
	if _, exists := users["testuser"].Folders["testfolder"].Files["testfile"]; exists {
		t.Errorf("File 'TESTFILE' and 'testfile' should be the same file.")
	}
	// test the rename command
	handleCommand("rename-folder TESTUSER TESTFOLDER NEWTESTFOLDER")
	if _, exists := users["testuser"].Folders["testfolder"]; exists {
		t.Errorf("Folder 'TESTFOLDER' and 'testfolder' should be the same folder.")
	}
	// test the delete folder command
	handleCommand("delete-folder TESTUSER NEWTESTFOLDER")
	if _, exists := users["testuser"].Folders["newtestfolder"]; exists {
		t.Errorf("Folder 'NEWTESTFOLDER' and 'newtestfolder' should be the same folder.")
	}
}
